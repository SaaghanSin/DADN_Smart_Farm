  ...
  const fetchLimits = async () => {
    try {
      const [baseLimitResponse, upperLimitResponse] = await Promise.all([
        axios.get("http://192.168.0.3:3000/base-limit"),
        axios.get("http://192.168.0.3:3000/upper-limit"),
      ]);
  ...
  const saveLimits = async () => {
    if (baseLimit < upperLimit) {
      if (
        ...
        try {
          await axios.put("http://192.168.0.3:3000/put-upper-limit", {
            upperLimit,
          });

          await axios.put("http://192.168.0.3:3000/put-base-limit", {
            baseLimit,
          });
    ...
    if (!isNaN(tempBase) && !isNaN(tempUpper)) {
      if (tempBase < tempUpper) {
        if (
          ...
          try {
            await axios.put("http://192.168.0.3:3000/put-form-limits", {
              tempBase,
              tempUpper,
            });
