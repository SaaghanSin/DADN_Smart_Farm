...
//ROUTES
//get all the light devices in the device table that have the type of light and have the name of the user
app.get("/light/:name", async (req, res) => {
    try {
        const username = req.params.name;
        const allLights = await pool.query("SELECT * FROM device WHERE device_type = 'light' AND username = $1", [username]);
...
app.post("/activity", async (req, res) => {
    try {
        ...
        const newActivity = await pool.query("INSERT INTO activity (activity_id, activity_time, acttivity_description, device_id) VALUES($1, $2, $3, $4) RETURNING *", [activity_id, activity_time, acttivity_description, device_id]);
...
app.post("/light/:name", async (req, res) => {
    try {
        ...
        const newLight = await pool.query("INSERT INTO device (device_id, device_type, device_location, username) VALUES($1, $2, $3, $4) RETURNING *", [device_id, device_type, device_location, username]);
...
app.delete("/light/:id", async (req, res) => {
    try {
        ...
        const deleteActivities = await pool.query("DELETE FROM activity WHERE device_id = $1", [id]);
        //then delete the light device that has the device id
        const deleteLight = await pool.query("DELETE FROM device WHERE device_id = $1", [id]);
